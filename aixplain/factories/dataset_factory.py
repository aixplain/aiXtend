...
class DatasetFactory(AssetFactory):
    ...
    def update(
        ...
        for i, data in enumerate(schema):
            f = [d for d in dataset_data if d.name == data.name]
            assert len(f) != 0, f"Data Asset Onboarding Error: The dataset has no data column called {data.name}."
            assert (
                len(f) == 1 or data.id is not None
            ), f"Data Asset Onboarding Error: The dataset has {len(f)} data columns called {data.name}. Please specify the ID of the column to be updated."

            if data.id is None:
                data.id = f[0].id
            assert (
                data.id in dataset_data_ids
            ), f"Data Asset Onboarding Error: Data `{data.id}` is not part of the Dataset `{dataset_id}`."
        ...
        try:
            ...
            for i in tqdm(range(len(schema)), desc=f" Dataset's onboard progress", position=0):
                ...
                files, data_column_idx, start_column_idx, end_column_idx, nrows = onboard_functions.process_data_files(
                    data_asset_name=metadata.name, metadata=metadata, paths=paths, folder=dataset.name
                )
